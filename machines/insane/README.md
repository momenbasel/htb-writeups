...
|---|---------|----|----|------|------|
| 0a | **Sorcery** | Linux | Cypher Injection + XSS Passkey Registration + Kafka Wire Protocol + FreeIPA | Cypher injection in Neo4j-backed app, abuse passkey registration XSS to register attacker WebAuthn credential, exploit Kafka wire protocol for lateral, FreeIPA misconfig for root | [0xdf](https://0xdf.gitlab.io/2026/04/25/htb-sorcery.html), [lazyhackers](https://lazyhackers.in/posts/sorcery-htb-writeup-hackthebox-season-8) |
| 0b | **Pterodactyl** | openSUSE | Pterodactyl Panel CVE-2025-49132 + PEAR pearcmd LFI + PAM/Polkit | Directory traversal in Pterodactyl panel reveals creds, abuse PEAR `pearcmd.php` LFI-to-RCE technique, escalate via PAM/Polkit chain for root | [0xdf](https://0xdf.gitlab.io/2026/05/16/htb-pterodactyl.html) |
| 0c | **Cobblestone** | Linux | Second-Order SQLi + LOAD_FILE + Twig SSTI + Cobbler XMLRPC CVE-2024-47533 | Vhost fuzz reveals vote+deploy subdomains, second-order SQLi on vote with LOAD_FILE to read server files, stored XSS for admin, Twig SSTI for RCE, Cobbler XMLRPC CVE-2024-47533 for root | [Wither2Rebirth](https://wither2rebirth.com/reports/htb-Cobblestone), [BenHeater](https://benheater.com/hackthebox-cobblestone/), [NoSec](https://nosecpwn.eu/cobblestone/en/), [Mane](https://manesec.github.io/2025/08/20/2025/57-hackthebox-Cobblestone-patch/), [CyberSecGuru](https://thecybersecguru.com/ctf-walkthroughs/mastering-cobblestone-beginners-guide-from-hackthebox/) |
...
| 13 | **Oouch** | Linux | OAuth CSRF chain, SSRF, DBus exploitation, uWSGI RCE | Chain OAuth flaws with SSRF to steal sessions, pivot through Docker via DBus | [0xdf](https://0xdf.gitlab.io/2020/08/01/htb-oouch.html), [hg8](https://hg8.sh/posts/oouch/), [snowscan](https://snowscan.io/htb-writeup-oouch/) |
...
| 15 | **Travel** | Linux | Memcache SSRF poisoning, PHP deserialization, LDAP privesc | Poison memcache via gopher SSRF with serialized PHP payload, escalate through LDAP | [snowscan](https://snowscan.io/htb-writeup-travel/), [chr0x6eos](https://chr0x6eos.github.io/2020/09/12/htb-Travel.html), [IppSec](https://www.youtube.com/watch?v=VofMBg2VLnw) |
...
| # | Machine | OS | Key Techniques | One-Line Summary | Writeup Links |
|---|---------|----|----|------|------|
| 0a | **MonitorsFour** | Windows | PHP Type Juggling + Cacti CVE + Docker API Abuse | PHP type juggling auth bypass on Cacti instance, exploit Cacti chain CVE for shell, abuse exposed Docker socket API for root container escape | [0xdf](https://0xdf.gitlab.io/2026/05/23/htb-monitorsfour.html) |
| 0b | **Overwatch** | Windows | .NET Reverse + WCF SOAP Service Injection + DNS Manipulation | Reverse .NET monitoring binary, inject into WCF SOAP service, manipulate DNS to redirect service calls for SYSTEM | [0xdf](https://0xdf.gitlab.io/2026/05/09/htb-overwatch.html) |
...
| 32 | **Sizzle** | Windows | SMB hash theft, ADCS cert abuse, Kerberoasting, DCSync | Steal NTLM hashes via writable SMB share, abuse certs for Kerberoast to DCSync | [InfoSecWriteups](https://infosecwriteups.com/hack-the-box-sizzle-write-up-62f3464701be), [0xRick](https://0xrick.github.io/hack-the-box/sizzle/), [snowscan](https://snowscan.io/htb-writeup-sizzle/), [IppSec](https://www.youtube.com/watch?v=YVhlfUvsqYc), [Medium](https://medium.com/@0xleksa/sizzle-hackthebox-detailed-writeup-4a92eac27780) |
...
| 39 | **Absolute** | Windows | Image metadata username enum, AD Kerberos-only attack chain | Extract usernames from image metadata, pure Kerberos exploitation in hardened AD environment | [HTB](https://www.hackthebox.com/machines/absolute), [IppSec](https://www.youtube.com/watch?v=rfAmMQV_wss) |
